...
<div class="container-field">
	<div class="col-lg-12">
		<div class="card">
			<div class="card-body">
				<?php if (isset($_SESSION['login_id'], $_SESSION['login_type']) && ($_SESSION['login_id'] || $_SESSION['login_type'] == 1)): ?>
					<div class="dropleft float-right mr-4" style="position: relative;">
						...
						<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
							<a class="dropdown-item edit_topic" data-id="<?php echo $id ?>" href="javascript:void(0)">Editar</a>
							<a class="dropdown-item delete_topic" data-id="<?php echo $id ?>" href="javascript:void(0)">Borrar</a>
						</div>
					</div>
				<?php endif; ?>
				<span class="float-right mr-4"><small><i><?php echo date('M d, Y h:i A', strtotime($date_created)) ?></i></small></span>
				<span class="float-right mr-4 text-primary"><small><i>Publicado por: <?php echo ucwords($name) ?></i></small></span>
				<div class="col-md-8">
					<h4><b><?php echo $title ?></b></h4>
				</div>
				<?php if (count($tags) > 0): ?>
					<div>
						<span class="badge badge-default"><i class="fa fa-tags"></i> Etiquetas: </span>
				...
				<hr>
				<div class="w-100">
					<span class="badge badge-secondary text-white"><?php echo number_format($view) ?> vista/s</span>
					<span class="badge badge-primary text-white ml-2"><i class="fa fa-comments"></i> <?php echo number_format(count($com_arr)) ?> comentario/s</span>
				</div>
		...
		<div class="card">
			<div class="card-body">
				<div class="col-lg-12">
					<div class="row">
						<h3><b> <i class="fa fa-comments"></i> Comentario/s</b></h3>
					</div>
						...
						<div class="form-group comment">
							<?php if ($_SESSION['login_id'] == $row['user_id']): ?>
								<div class="dropleft float-right">
									...
									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										<a class="dropdown-item edit_comment" data-id="<?php echo $row['id'] ?>" href="javascript:void(0)">Editar</a>
										<a class="dropdown-item delete_comment" data-id="<?php echo $row['id'] ?>" href="javascript:void(0)">Borrar</a>
									</div>
								</div>
							<?php endif; ?>
							<span class="float-right mr-4"><small><i>Creado: <?php echo date('M d, Y h:i A', strtotime($row['date_created'])) ?></i></small></span>
							...
							<div>
								...
								<?php if (isset($rep_arr[$row['id']])): ?>
									...
									<div class="col-lg-8 offset-lg-2 replies">
										<a href="javascript:void(0)" class="show_all" style="display: none">Mostrar todas las respuestas</a>
										<?php
											...
											<div class="form-group ty-compact-list">
												<?php if ($_SESSION['login_id'] == $rep['user_id']): ?>
													<div class="dropleft float-right">
														...
														<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
															<a class="dropdown-item edit_reply" data-id="<?php echo $rep['id'] ?>" href="javascript:void(0)">Editar</a>
															<a class="dropdown-item delete_reply" data-id="<?php echo $rep['id'] ?>" href="javascript:void(0)">Borrar</a>
														</div>
													</div>
												<?php endif; ?>
												<span class="float-right mr-4"><small><i>Creado: <?php echo date('M d, Y h:i A', strtotime($rep['date_created'])) ?></i></small></span>
				...
				<div class="col-lg-12">
					<form action="" id="manage-comment">
						<div class="form-group">
							...
							<textarea class="form-control jqte" id="comment-txt" name="comment" cols="30" rows="5" placeholder="Nuevo comentario"></textarea>
						</div>
	...
	<div class="col-lg-8 offset-lg-2 reply-field">
		...
		<form action="" id="">
			<div class="form-group">
				...
				<textarea class="form-control" name="" cols="30" rows="5" placeholder="Nueva respuesta"></textarea>
			</div>
			<button class="btn btn-primary">Respuesta</button>
		</form>
	</div>
</div>
<script>
	...
	$('.replies .show_all').click(function() {
		...
		if ($(this).text() == 'Mostrar todas las respuestas')
			$(this).text('Mostrar menos')
		else
			$(this).text('Mostrar todas las respuestas')
	})
	// }
	...
	$('.delete_topic').click(function() {
		_conf("¿Está seguro de eliminar este tema?", "delete_topic", [$(this).attr('data-id')], 'mid-large')
	})
	...
	$('.delete_comment').click(function() {
		_conf("¿Estás seguro de eliminar este comentario?", "delete_comment", [$(this).attr('data-id')], 'mid-large')
	})
	...
	$('.delete_reply').click(function() {
		_conf("¿Está seguro de eliminar esta respuesta?", "delete_reply", [$(this).attr('data-id')], 'mid-large')
	})
